...
struct ClientEmitterTests {
  ...
  @Test
  func emitsBinaryRequestBodyOperation() {
    let document = IRDocument(
      schemas: [],
      operations: [
        IROperation(
          ...
          path: "/{functionName}",
          parameters: [
            IRParameter(name: "functionName", location: .path, type: .string, isOptional: false)
          ],
          requestBody: .binary,
          responses: [
            IRResponse(statusCode: 200, isError: false, body: .binary)
          ]
        )
      ]
    )
