...
public enum SwiftEmitter {
  ...
  public static func emitModels(
    _ document: IRDocument, namespace: String, accessLevel: AccessLevel = .internal
  ) -> String {
    ...
    var lines: [String] = [
      ...
      "",
      "\(accessLevel.rawValue) enum \(namespace) {",
    ]
  ...
  static func indented(_ text: String, by spaces: Int = 2) -> String {
  ...
  public static func emitClient(
    _ document: IRDocument, namespace: String,
    accessLevel: AccessLevel = .internal
  ) -> String {
    let errorSchemaNames = self.errorSchemaNames(in: document)
    let importKeyword = accessLevel == .public ? "public import" : "import"
    var structLines: [String] = [
      "\(accessLevel.rawValue) struct Client: Sendable {",
    ...
    let lines: [String] = [
      ...
      "\(importKeyword) HTTPRuntime",
      "",
      "\(accessLevel.rawValue) extension \(namespace) {",
      indented(structLines.joined(separator: "\n")),
      "}",
    ]
    return lines.joined(separator: "\n")
  }
  ...
  static func emitOperation(
    ...
    let mutatesBuilder =
      parameters.contains { $0.location == .query || $0.location == .header }
      || operation.requestBody != nil
    var lines: [String] = [
      "  \(accessLevel.rawValue) func \(methodName)(\(signatureParts.joined(separator: ", "))) async throws"
        + (returnType.map { " -> \($0)" } ?? "") + " {",
      "    \(mutatesBuilder ? "var" : "let") builder = HTTPRequestBuilder(method: .\(operation.method.rawValue), baseURL: baseURL, path: \"\(pathTemplate(operation))\")",
    ]
