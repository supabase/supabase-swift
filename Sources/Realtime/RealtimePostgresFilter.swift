...
public enum RealtimePostgresFilter {
  ...
  /// The serialized `column=operator.value` fragment for a single condition.
  ///
  /// `nil` for composite cases (``not(_:)`` and ``and(_:)``), which cannot
  ...
  var value: String {
    ...
    default:
      // Safe: `parts` is only nil for `.not`/`.and`, handled above.
      let parts = parts!
  ...
  /// Reserved characters that force PostgREST-style quoting (condition/list
  /// delimiters plus the quote/escape characters themselves).
  ...
  /// Removes duplicate values while preserving order (matches supabase-js `in`).
