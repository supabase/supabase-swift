...
// These tests are Darwin-only because swift-corelibs-foundation (Linux) crashes when a custom
// URLProtocol (e.g. MockingURLProtocol) intercepts a URLSessionDownloadTask.
// The crash is a forced cast in _ProtocolClient.urlProtocolDidFinishLoading → completeTask
// at Sources/FoundationNetworking/URLSession/URLSessionTask.swift:1177:
//
//   let temporaryFileURL = urlProtocol.properties[URLProtocol._PropertyKey.temporaryFileURL] as! URL
//
// MockingURLProtocol delivers an in-memory response and never writes a temp file, so the
// property is nil and the forced cast traps with SIGILL.
// See: https://github.com/swiftlang/swift-corelibs-foundation/blob/main/Sources/FoundationNetworking/URLSession/URLSessionTask.swift#L1177
#if canImport(Darwin)
