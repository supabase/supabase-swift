  ...
  final class StorageBucketAPITests: XCTestCase {
    ...
    func testURLConstruction() {
      ...
      for (input, expect, description) in urlTestCases {
        ...
        XCTAssertEqual(
          storage.configuration.url.absoluteString, expect,
          "should \(description) if useNewHostname is true")

        let storage2 = SupabaseStorageClient(
          configuration: StorageClientConfiguration(
            url: URL(string: input)!,
            headers: [:],
        ...
        XCTAssertEqual(
          storage2.configuration.url.absoluteString, input,
          "should not modify host if useNewHostname is false")
      }
    }
